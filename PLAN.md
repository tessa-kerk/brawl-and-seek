...
- **Result (measured):** escape sweep now 0 stuck / 0 clip across 296 (tile,dir) cases with a *moderate* drag inside a 100ms window; a 5px nudge moves ~10px/100ms; multi-touch handoff works; keyboard/paint-timer/break regression all green; zero console errors.
...
**BUILD STAMP RITUAL (every commit):** bump `CFG.BUILD.n` in `src/config.js` AND find-replace `?v=<old>`→`?v=<new>` across `index.html` (currently **`?v=2`**, BUILD n:2). The top-right title stamp (`v2 · M1`) is the witness — if it doesn't match, a script is cached stale.
