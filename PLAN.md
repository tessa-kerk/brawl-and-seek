...
**BUILD STAMP RITUAL (every commit):** bump `CFG.BUILD.n` in `src/config.js` AND find-replace `?v=<old>`→`?v=<new>` across `index.html` (currently **`?v=3`**, BUILD n:3). The top-right title stamp (`v3 · M1`) is the witness — if it doesn't match, a script is cached stale.
