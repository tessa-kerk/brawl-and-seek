...
with game() as (pg, errs):
    ...
    # clean floor run cols3-9 (the section's longest clear sightline), used for
    # every open-path check below. The wall-block check uses the genuine
    # interior `#` cluster at row8, cols1-3 instead.

    # A) travel speed ~2x base
    pos = setup(3, 5, 9, 5); fire(pos, pos["sx"] + 300, pos["sy"])
    ...
    # B) range ~3 tiles, then a MISS costing 30 — dummy parked on a different
    # row (row2) so the rightward shot along row5 can never reach it; it just
    # runs out of range over open floor.
    pos = setup(3, 5, 8, 2); fire(pos, pos["sx"] + 300, pos["sy"]); trav = 0
    for _ in range(60):
        s = st()
        if s["n"] == 0: break
        trav = s["trav"]; step()
    ...
    # C) walls block it; wall hit is a wrong tag. The section's only genuine
    # interior wall cluster sits at row8, cols1-3 (floor at col4+ to its right,
    # the pool's second lobe at col0 to its left) — seeker on the floor side
    # fires left at a coordinate beyond the cluster, straight through it.
    pos = setup(4, 8, 0, 8); fire(pos, pos["dx"], pos["dy"])
    for _ in range(60):
        if st()["n"] == 0: break
        step()
    s = st()
    ...
    for hidden, label in [(True, "camouflaged"), (False, "running")]:
        pos = setup(3, 5, 6, 5, hidden); fire(pos, pos["dx"], pos["dy"])
        for _ in range(60):
            if st()["n"] == 0: break
            step()
        s = st()
    ...
    # F) 0.75s cooldown — a 2-tile gap, safely inside range*fireAt (3.0*0.92=2.76
    # tiles) so the chase AI's inRange check actually commits to a shot.
